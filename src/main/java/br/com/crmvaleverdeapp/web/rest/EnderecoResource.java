...
/**
 * REST controller for managing {@link br.com.crmvaleverdeapp.domain.Endereco}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EnderecoResource {
    ...
    public List<Endereco> getAllEnderecos(@RequestParam(required = false) String filter) {
        if ("contato-is-null".equals(filter)) {
            log.debug("REST request to get all Enderecos where contato is null");
            return StreamSupport
                .stream(enderecoRepository.findAll().spliterator(), false)
                .filter(endereco -> endereco.getContato() == null)
