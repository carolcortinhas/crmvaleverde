...
/**
 * REST controller for managing {@link br.com.crmvaleverdeapp.domain.Email}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EmailResource {
     ...
     */
    @GetMapping("/emails")
    public List<Email> getAllEmails(@RequestParam(required = false) String filter) {
        if ("contato-is-null".equals(filter)) {
            log.debug("REST request to get all Emails where contato is null");
            return StreamSupport
                .stream(emailRepository.findAll().spliterator(), false)
                .filter(email -> email.getContato() == null)
