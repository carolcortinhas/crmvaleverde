...
import br.com.crmvaleverdeapp.repository.PessoaRepository;
...
/**
 * REST controller for managing {@link br.com.crmvaleverdeapp.domain.Pessoa}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PessoaResource {
     ...
     */
    @GetMapping("/pessoas")
    public List<Pessoa> getAllPessoas(@RequestParam(required = false) String filter) {
        if ("representante-is-null".equals(filter)) {
            log.debug("REST request to get all Pessoas where representante is null");
            return StreamSupport
                .stream(pessoaRepository.findAll().spliterator(), false)
                .filter(pessoa -> pessoa.getRepresentante() == null)
