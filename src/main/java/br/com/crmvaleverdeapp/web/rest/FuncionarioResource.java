...
import br.com.crmvaleverdeapp.domain.Funcionario;
import br.com.crmvaleverdeapp.repository.FuncionarioRepository;
...
/**
 * REST controller for managing {@link br.com.crmvaleverdeapp.domain.Funcionario}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class FuncionarioResource {
    ...
    public List<Funcionario> getAllFuncionarios(@RequestParam(required = false) String filter) {
        if ("pessoa-is-null".equals(filter)) {
            log.debug("REST request to get all Funcionarios where pessoa is null");
            return StreamSupport
                .stream(funcionarioRepository.findAll().spliterator(), false)
                .filter(funcionario -> funcionario.getPessoa() == null)
